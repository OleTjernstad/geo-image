...
module.exports = (grunt) => {
  ...
  grunt.initConfig({
    ...
    concat: {
      license: {
        ...
        files: {
          'dist/image-plugin/plugin.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ],
          'dist/image-plugin/plugin.min.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ]
        }
    ...
    copy: {
      css: {
        files: [
          { src: [ 'CHANGELOG.txt', 'LICENSE.txt' ], dest: 'dist/image-plugin', expand: true }
        ]
      }
    },
  ...
  grunt.registerTask('version', 'Creates a version file', () => {
    grunt.file.write('dist/image-plugin/version.txt', BUILD_VERSION);
  });
